...
def split_channels_into_tiles(stitched_dirs: dict, new_tiles_per_cycle_region: dict, block_size=1000, overlap=50):
    for cycle in stitched_dirs:
        for region in stitched_dirs[cycle]:
            for channel, dir_path in stitched_dirs[cycle][region].items():
                ...
                slice_tiff(path_to_str(stitched_image_path), out_dir, block_size=block_size, overlap=overlap, region=region, zplane=1, channel=channel)

...
def get_values_from_pipeline_config(pipeline_config: dict) -> dict:
    info_for_bigstitcher = dict(dataset_dir=Path(pipeline_config['raw_data_location']),
...
def run_bigstitcher(bigstitcher_macro_path: Path):
    ...
    if res.returncode == 0:
        ...
        raise Exception('There was an error while running the BigStitcher: \n' + res.stderr.decode('utf-8'))


def run_bigstitcher_for_ref_channel_per_region(ref_channel_dir_per_region: dict, ref_channel_stitched_dir_per_region: dict, info_for_bigstitcher: dict):
    for region, dir_path in ref_channel_dir_per_region.items():
        ...
        bigstitcher_macro_path = generate_bigstitcher_macro_for_reference_channel(ref_channel_dir, ref_channel_stitched_dir, info_for_bigstitcher, region)
...
def copy_bigsticher_files_to_dirs(channel_dirs: dict, stitched_channel_dirs: dict, ref_channel_dir_per_region: dict):
...
def get_ref_channel_dir_per_region(channel_dirs: dict, stitched_channel_dirs: dict, num_channels_per_cycle: int, reference_channel_id: int):
...
def main(data_dir: Path, pipeline_config_path: Path):
    ...
    ref_ch_dirs = get_ref_channel_dir_per_region(channel_dirs, stitched_channel_dirs, num_channels_per_cycle, ref_channel_id)
    ref_channel_dir_per_region, ref_channel_stitched_dir_per_region = ref_ch_dirs

    print('\nEstimating stitching parameters')
    run_bigstitcher_for_ref_channel_per_region(ref_channel_dir_per_region,
                                               ref_channel_stitched_dir_per_region, dataset_meta)

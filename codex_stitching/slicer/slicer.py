...
def split_by_size(arr: np.ndarray, region: int, zplane: int, channel: int, tile_w: int, tile_h: int, overlap: int):
...
def slice_img(in_path: str, out_dir: str, tile_size: int, overlap: int, region: int, channel: int, zplane: int):

    this_plane_tiles, this_plane_img_names = split_by_size(
        tif.imread(in_path), region=region, zplane=zplane, channel=channel, tile_w=tile_size, tile_h=tile_size, overlap=overlap
    )

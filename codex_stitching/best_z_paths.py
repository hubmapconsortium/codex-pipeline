...
def _get_reference_channel_paths(listing_per_cycle: dict, num_channels_per_cycle: int, reference_channel_id: int) -> Dict[int, Path]:
...
def _create_dirs_for_each_channel(listing_per_cycle: dict, out_dir: Path) -> Dict[int, Dict[int, Dict[int, Dict[int, Dict[int, Path]]]]]:
    ...
    for cycle in listing_per_cycle:
        ...
        for region in this_cycle_listing:
            ...
            for channel in this_region_listing:
                channel_dir_name = naming_template.format(cycle=cycle, region=region, channel=channel)
...
def _find_best_z_planes_per_region_tile(reference_channel_tile_paths: dict, max_z: int, x_ntiles: int, y_ntiles: int, tiling_mode: str) -> Dict[int, Dict[int, List[Path]]]:
    best_z_plane_per_region = dict()

    for region in reference_channel_tile_paths:
        best_z_plane_per_tile = get_best_z_plane_ids_per_tile(reference_channel_tile_paths[region], x_ntiles, y_ntiles, max_z, tiling_mode)
        best_z_plane_per_region[region] = best_z_plane_per_tile

    return best_z_plane_per_region


def _select_best_z_planes_in_this_channel(channel_paths: dict, out_dir: Path, best_z_plane_per_tile: dict) -> List[Tuple[List[Path], Path]]:
...
def _select_best_z_plane_paths(listing_per_cycle: dict, channel_dirs: dict, best_z_plane_per_region: dict) -> Dict[int, Dict[int, Dict[int, List[Tuple[List[Path], Path]]]]]:
    ...
    for cycle in listing_per_cycle:
        ...
        for region in this_cycle_listing:
            ...
            for channel in this_region_listing:
                ...
                best_z_plane_paths = _select_best_z_planes_in_this_channel(this_channel_paths, this_channel_out_dir,
                                                                           this_region_best_z_planes)
...
def get_output_dirs_and_paths(img_dirs: List[Path], out_dir: Path, num_channels_per_cycle: int, max_z: int, x_ntiles: int, y_ntiles: int, tiling_mode: str, reference_channel_id: int):
    listing_per_cycle = create_listing_for_each_cycle_region(img_dirs)
    reference_channel_tile_paths = _get_reference_channel_paths(listing_per_cycle, num_channels_per_cycle, reference_channel_id)
    channel_dirs = _create_dirs_for_each_channel(listing_per_cycle, out_dir)
    best_z_plane_per_region = _find_best_z_planes_per_region_tile(reference_channel_tile_paths, max_z, x_ntiles, y_ntiles, tiling_mode)
    best_z_plane_paths = _select_best_z_plane_paths(listing_per_cycle, channel_dirs, best_z_plane_per_region)

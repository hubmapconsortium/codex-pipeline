...
def generate_slicer_info(tile_shape_no_overlap: Tuple[int, int], overlap: int, stitched_img_shape: Tuple[int, int]) -> dict:
    slicer_info = dict()
    ...
    x_ntiles = img_width // tile_width if img_width % tile_width == 0 else (img_width // tile_width) + 1
    y_ntiles = img_height // tile_height if img_height % tile_height == 0 else (img_height // tile_height) + 1
    ...
    slicer_info['slicer']['tile_shape_with_overlap'] = {'x': tile_width + overlap * 2, 'y': tile_height + overlap * 2}
    return slicer_info
...
def replace_values_in_config(exp, slicer_info):
    ...
    values_to_replace = {'tiling_mode': 'grid',
                         ...
                         'target_shape': [slicer_info['slicer']['tile_shape_no_overlap']['x'],
...
def modify_pipeline_config(path_to_config: Path, tile_shape_no_overlap: Tuple[int, int], overlap: int, stitched_img_shape: Tuple[int, int]):
